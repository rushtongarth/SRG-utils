...
def restart_dboxd(pause=0):
    if is_dropbox_running():
        stop([])
    if pause:
        console_print(u'resting for %s sec...'%pause)
        time.sleep(pause)
    start([])
...
def refresh(argv):
    u"""restart dropbox
dropbox restart
dropbox restart --sleep
dropbox restart --sleep 2

options:
  -s --sleep  seconds to sleep prior to restart (default 5)

restarts dropbox with option to sleep for a specified number of 
seconds.
"""
    if len(argv) == 0:
       restart_dboxd()
       ...
       restart_dboxd(5)
    elif len(argv) == 2 and argv[0].startswith('-') and argv[1].isdigit():
       s = int(argv[1])
       restart_dboxd(s)
    else:
       console_print(''.join(refresh.__doc__.split('\n', 1)[1:]).decode('ascii'))
